   ...
   public function login(Request $request)
    ...
    public function register(Request $request)
    {
        ...
        if($checkemail){
            ...
            $message="تم تسجيل دكتور جديد بأسم  ";
            $doctor = Doctor::where('id',$add->id)->first();  
    ...
    public function getDoctorData(Request $request)
    {
        ...
        $serviceName= Service::selection()->where('doctorId',$request->id)->where('status',1)->get();
        foreach ($serviceName as $itemmm) {
            $durationdoctorrrr= WorkingDays::where('doctorId',$itemmm->doctorId)->first(); 
            if($durationdoctorrrr){
                $itemmm->duration=$durationdoctorrrr->duration;
            }else{
                $itemmm->duration="0";
            }  
        }  
