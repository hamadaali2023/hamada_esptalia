...
{  
    ...
    public function index(Request $request ,$radius = 400)
    {
        ...
        foreach ($doctors as $item) {
            ...
            $sum_review=Reviews::where('doctorId',$item->id)->sum('rate');
            $allreview=Reviews::where('doctorId',$item->id)->get();
    ...
    public function getDoctorsSpecialityById(Request $request)
    {
        ...
        // $doctors = Doctor::selection()->get();
        // dd($category);
        $doctors= Doctor::selection()->where('status',1)->where('specialityId',$request->id)->get();       
        foreach ($doctors as $item) {
            $item->specialityName= Speciality::selection()->where('id',$item->specialityId)->first();  
            $item->serviceName= Service::selection()->where('doctorId',$item->id)->where('status',1)->get();
            ...
            // $durationdoctorrrr= WorkingDays::where('doctorId',$item->id)->first(); 
            // $item->duration=$durationdoctorrrr->duration;
    ...
    public function getTime(Request $request){
        ...
        $day = Carbon::createFromFormat('Y-m-d', $request->date)->dayOfWeek;
        //   dd($day);
        $times=WorkingDays::where('day_number', $day)->where('doctorId', $request->doctorId)->get();
        // dd($times);
    ...
    public function addPayment(Request $request)
    {
            ...
            $message="تم حجز موعد من قبل  ";

            $patient->notify(new \App\Notifications\PatientNewNotification($patient->first_name_ar,$patient->id,$message,$patient->photo,$add->offer_name_ar,$add->description_ar));
    ...
    public function getAppointmentById(Request $request)
    {
        ...
        foreach ($appointments as $item) {
            ...
            $sum_review=Reviews::where('doctorId',$item->doctorId)->sum('rate');
            $allreview=Reviews::where('doctorId',$item->doctorId)->get();
            $count_review= count($allreview);
    ...
    {
        $reviews=Reviews::where('doctorId',$request->doctorId)->get();
